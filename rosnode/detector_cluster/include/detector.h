...
public:
    ...
    MyPoint get_piles(std::vector<BrickLine> &lines);
    ...
    bool check_pile(std::vector<BrickLine> &lines, MyPoint &pile, int min_num, float max_height, float max_dist);

    std::array<MyPoint, 4> fit_clusters(std::array<std::vector<MyPoint>, 4> points, std::array<std::vector<MyPoint>, 4> alphas, std::array<float, 4> k);
