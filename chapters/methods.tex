...
\begin{equation}
\alpha_n = P(x_n | \mu, \sigma),
\end{equation}
when there are more classes $Z$, this is calculated using the Bayes theorem and the law of total probability:
\begin{align}
\alpha_{n, z = k} &= \dfrac{P(x_n | z = k; \mu, \sigma) P(z=k)}{P(x_n)}, \nonumber \\
\alpha_{n, z = k} &= \dfrac{P(x_n | z = k; \mu, \sigma) P(z=k)}{\sum_{j = 1}^Z P(x_n | z = j ; \mu, \sigma) P(z = j)}.
\end{align}
There is only one class in our case, so we need to obtain the probability of the current sample, and we finally can get the output of the expectation step:
\begin{equation}
