...
\begin{table}[H]
	...
	\begin{tabular}{ccccccc}
		...
		Rear model &\quad& 17 &\quad& 0 &\quad& 100\% \\
		\bottomrule
	\end{tabular}
...
As we already mentioned the ground segmentation applied in this experiment was not sufficient enough. The slope inside the area was responsible for high number of false positive detections. The easiest possible ground segmentation method was used in the detector, because we did not expected such a tough terrain. The height of lidar above the ground was $0.45$m, so all points with $z$ coordinate lower than $-0.4$ was filtered out of the measurement. This method can segment only completely flat ground, which was not the case of the arena.

Ground segmentation is popular subject of study. There are various published papers concerning this topic. Some methods are incorporating machine learning \cite{velas2018}. The usage of machine learning again involves evaluating of neural networks and that can be inefficient on CPU. There are also purely algorithmic methods which perform well even in rough terrain \cite{phuong2019}. These methods does not incorporate any knowledge about the environment which is not completely our case, because the arena terrain is known in advance.

We had available advanced mapping device (Leica BLK 360), which is able to create model of arena using very dense pointcloud. Such a pointcloud can be than used to create elevation map of the arena. This elevation map can help to segment out the ground in the robot measurements. Firstly would be necessary to implement queries to such a map. Then the robot could obtain transformation of pointcloud using its position. When the candidates are generated it could be verified whether the candidate are above the terrain. This would work reasonably well until the localization of robot works fine.
